...
export const VideoComposition: React.FC<VideoCompositionProps> = ({
  ...
  // 预计算每个场景的 globalFrameOffset（避免在 JSX 渲染中累加导致不确定性）
  ...
  const totalDurationInFrames = cumulativeOffset;

  // 调试日志：全局背景视频循环
  console.log(`[VideoComposition] 全局背景:`, {
    isBookAnalysis,
    backgroundUrl,
    backgroundType,
    backgroundVideoDurationInFrames,
  ...
  // 调试日志：场景数据和帧偏移
  console.log(`[VideoComposition] 场景总数: ${activeShots.length}`);
  activeShots.forEach((shot, i) => {
    console.log(`[VideoComposition] Shot ${shot.index}:`, {
